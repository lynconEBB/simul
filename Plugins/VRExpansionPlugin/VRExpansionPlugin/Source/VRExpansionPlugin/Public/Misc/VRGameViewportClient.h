...
public:
	UVRGameViewportClient(const FObjectInitializer& ObjectInitializer)
		: Super(ObjectInitializer)
	{
		GameInputMethod = EVRGameInputMethod::GameInput_KeyboardAndMouseToPlayer2;
	...
	virtual bool InputKey(const FInputKeyEventArgs& EventArgs) override
	{
		...
		if (GameInputMethod == EVRGameInputMethod::GameInput_Default || IgnoreInput() || (EventArgs.IsGamepad() && !
			IsValidGamePadKey(EventArgs.Key)))
	...
	virtual bool InputAxis(FViewport* tViewport, int32 ControllerId, FKey Key, float Delta, float DeltaTime,
	                       int32 NumSamples = 1, bool bGamepad = false) override
	{
		...
		if (((!Key.IsMouseButton() && !bGamepad) || (bGamepad && !IsValidGamePadKey(Key))) || NumLocalPlayers < 2 ||
			GameInputMethod == EVRGameInputMethod::GameInput_Default || IgnoreInput())
